    ...
    public partial class Form1 : Form
    {
        ...
        private void StoreData(byte[] content)
        {
            ...
            try
            {
                ...
                string SQLString = "Insert Into data(NumberCar,DateIn,Image) values (@NumberCar,@DateIn,@Image)";
                OleDbCommand insert = new OleDbCommand(SQLString, conn);

                OleDbParameter param = new OleDbParameter("@NumberCar", OleDbType.Integer);
                param.Value = lptext.Text;
                insert.Parameters.Add(param);

                OleDbParameter param1 = new OleDbParameter("@DateIn", OleDbType.Date);
                param.Value = indate.Text;
                insert.Parameters.Add(param1);

                //param = new OleDbParameter("@DateOut", OleDbType.Date);
                //param.Value = type;
                //insert.Parameters.Add(param);

                OleDbParameter param2 = new OleDbParameter("@Image", OleDbType.Binary);
                param.Value = content;
