...
namespace OCRSharp
{
    public partial class Form1 : Form
    {
        ...
        private void Recog()
        {
            OCR.x = uint.Parse(areax.Text);
            OCR.y = uint.Parse(areay.Text);
            OCR.PreProcess();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if(sender==clock)
            {
                indate.Text = DateTime.Now.ToString();
                int deadtime;
                deadtime = DateTime.Now.Second;// +DateTime.Now.Minute * 60;
                if (((deadtime - temp) > 15) && ((deadtime - temp) < 20))
                {
                    MessageBox.Show("HE THONG DANG BI TRE");
        ...
        private void FillCombo()
        {
            try
            {
                ...
                foreach( DataRow drow in dtable.Rows)
                {
                    ...
                    //cmbPos.Items.Add(drow[5].ToString());
        ...
        private void receivecar()
        {
            ...
            {    
                    ...
                    SendData(postext.Text.ToString());
                    // delete sql query
                    string queryDeleteString = "DELETE FROM data WHERE data.IDcard LIKE '" + name + "%'";
                    OleDbCommand sqlDelete = new OleDbCommand();
                    sqlDelete.CommandText = queryDeleteString;
                    sqlDelete.Connection = database;
                    sqlDelete.ExecuteNonQuery();
